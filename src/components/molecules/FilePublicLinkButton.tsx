...
import { copyPublicLinkToClipboard, hasPublicLink } from '../../services/fileService';
  ...
  return (
    <Tooltip title={showSuccess ? 'Copied' : tooltip}>
      <IconButton 
        ...
        sx={{ 
          p: { xs: 0.75, md: 0.5 },
          color: showSuccess ? 'success.main' : (hasLink ? 'primary.main' : 'inherit'),
          '&:hover': {
      ...
      >
        {showSuccess ? <CheckIcon fontSize={size} /> : <LinkIcon fontSize={size} />}
      </IconButton>
    </Tooltip>
  );
};
